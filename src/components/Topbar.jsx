...
import { LogOut, Moon, Sun, Maximize2, Minimize2, Menu, ChevronDown, Building2, Bell } from 'lucide-react'
...
export default function Topbar({ onToggleSidebar }) {
  ...
  return (
    <header className="sticky top-0 z-40 h-16 flex items-center justify-between px-4 md:px-6 flex-shrink-0 bg-white/95 dark:bg-[#0d1322]/95 backdrop-blur-md border-b border-slate-200/70 dark:border-slate-800/80 shadow-sm transition-colors">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <h2 className="text-sm md:text-base font-extrabold text-slate-900 dark:text-white tracking-tight truncate">
            IGH Business Management
          </h2>
          <span className="hidden sm:inline-flex text-[11px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/50 border border-blue-200/70 dark:border-blue-800/50 rounded-full px-2.5 py-0.5">
            System
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-2.5">
        ...
        {user?.role === 'admin' && (
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-bold transition-all shadow-sm ${bc.bg}`}>
            <Building2 size={13} className="flex-shrink-0" />
            <select
              value={selectedBranch}
              onChange={(e) => setSelectedBranch(e.target.value)}
              className="bg-transparent border-none text-xs font-bold focus:ring-0 cursor-pointer p-0 pr-3 appearance-none text-inherit"
        ...
        <div className="hidden md:flex items-center gap-2.5 pl-2 pr-3 py-1 rounded-full bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 dark:border-slate-700/60 shadow-sm">
          ...
          <div className="text-left leading-tight">
            ...
            <p className="text-[10px] font-semibold text-slate-400 dark:text-slate-500 capitalize">
        ...
        <button
          onClick={logout}
          className="btn-danger px-3 py-1.5 text-xs font-bold rounded-xl gap-1.5"
          title="Logout"
        >
          <LogOut size={14} />
